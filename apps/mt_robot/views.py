...
from .settings import CACHE_TIMEOUT,SITEMAP_URLS,USE_SITEMAP
...
def get_robots(request):
    ...
    def reverse_sitemap_url():
        return reverse('django.contrib.sitemaps.views.sitemap')

    def get_sitemap_urls():
        sitemap_urls = SITEMAP_URLS

        if not sitemap_urls and USE_SITEMAP:
            scheme = request.is_secure() and 'https' or 'http'
            sitemap_url = reverse_sitemap_url()
            if sitemap_url is not None:
                if not sitemap_url.startswith(('http', 'https')):
                    sitemap_url = "%s://%s%s" % (scheme, Site.objects.get_current().domain, sitemap_url)
    ...
    rules = Rule.objects.filter(sites=Site.objects.get_current())
    sitemap_urls = get_sitemap_urls()
    return render(request,
            'mt_robot/rule_list.html',
            {'rules': rules,'sitemap_urls': sitemap_urls,},content_type='text/plain')
