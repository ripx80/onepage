...
class ShortenSlug(AutoSlugField):
    '''Create a Short encoded Slug'''
    def slugify_func(self, content):
        if content:
            return base64.urlsafe_b64encode(hashlib.md5(content).digest())[:10]
        return ''

def get_short(request,obj):
    scheme=request.is_secure() and 'https' or 'http'
    short=obj.short and obj.short or obj.slug
    return scheme+'://'+request.get_host()+'/'+short
